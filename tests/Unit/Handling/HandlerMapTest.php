...
final class HandlerMapTest extends TestCase
{
    ...
    public function testFindIsCaseInsensitiveForMethod(): void
    {
        $map = new HandlerMap();
        $handler = Handler::status(200);
        $patternHandler = Handler::status(204);

        $map->forPath('/pets', 'get', $handler);
        $map->forPath('/pets/{petId}', 'DELETE', $patternHandler);

        self::assertSame($handler, $map->find('', '/pets', 'GET'));
        self::assertSame($handler, $map->find('', '/pets', 'get'));
        self::assertSame($patternHandler, $map->find('', '/pets/1', 'delete', '/pets/{petId}'));
    }
