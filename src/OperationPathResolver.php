...
{
    private ServerUrlMatcher $urlMatcher;

    /**
     * Create a resolver with a shared server URL matcher.
     */
    public function __construct(?ServerUrlMatcher $urlMatcher = null)
    {
        $this->urlMatcher = $urlMatcher ?? new ServerUrlMatcher();
    }
    ...
    {
        $path = $this->urlMatcher->normalizePath($request->getUri()->getPath());
        ...
        foreach ($schema->serverUrls() as $serverUrl) {
            ...
            if (!is_array($base) || !$this->urlMatcher->matchesRequest($request, $base)) {
                continue;
            }

            $basePath = $this->urlMatcher->normalizePath((string) ($base['path'] ?? '/'));
            $operationPath = $this->urlMatcher->stripBasePath($path, $basePath);
