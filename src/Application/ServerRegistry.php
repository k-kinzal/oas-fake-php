...
final class ServerRegistry
{
    ...
    private ?InterceptorRouter $router = null;

    private ?VcrLifecycle $vcrLifecycle = null;
    ...
    {
        if (($this->servers[$key] ?? null) === $server) {
            return;
        }
        ...
        if ($interceptor !== null) {
            $router = $this->router ??= new InterceptorRouter(new ServerUrlMatcher());
            $router->add($key, $server->serverUrls(), $interceptor, $server->resolveMode());
        }

        $vcrLifecycle = $this->vcrLifecycle ??= new VcrLifecycle();
        $vcrLifecycle->activate(fn (VcrRequest $request): VcrResponse => $this->dispatch($request));
    }
    ...
    public function unregister(string $key): void
    {
        ...
        if ($this->servers === [] && $this->vcrLifecycle !== null) {
